...
import { CustomMapRef, ParkingSpotType } from "@/types";
...
const CustomMap: React.FC<CustomMapProps> = ({ onLocationChange }) => {
  ...
  useEffect(() => {
    const searchPlaces = async (
      ...
      if (text.length > 0) {
        try {
          // Search through parking spots first with null checks
          const matchingParkingSpots = parkingSpots
            .filter((spot) => {
              // Add null checks for parkingName and address
              const parkingName = spot.parkingName?.toLowerCase() || "";
              const address = spot.address?.toLowerCase() || "";
              const searchText = text.toLowerCase();

              return (
                parkingName.includes(searchText) || address.includes(searchText)
              );
            })
            .map((spot) => ({
              place_id: spot.id,
  ...
  const getCurrentLocation = async () => {
    try {
      ...
      // Update nearby spots with new location
      ...
      {routeInfo && (
        <View style={styles.routeInfoContainer}>
          ...
          <View style={styles.transportInfo}>
            ...
            <View style={styles.transportItem}>
              <FontAwesome5 name="bicycle" size={20} color={colors.primary} />
              <Text style={styles.transportTime}>

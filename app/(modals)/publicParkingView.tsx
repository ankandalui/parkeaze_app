...
const PublicParkingView: React.FC = () => {
  ...
  const parseSlotStatus = (statusArray: string[]): string[] => {
    return statusArray.map((status: string) => {
      return status.includes("Filled") ? "Filled" : "Empty";
    });
  };

  const countAvailableSpots = (slots: string[]): number => {
    return slots.filter((status: string) => status === "Empty").length;
  };
    ...
    const connectSocket = () => {
      ...
      socket.on("update_data", (data: ParkingData) => {
        ...
        if (data && Array.isArray(data.output)) {
          const parsedStatuses = parseSlotStatus(data.output);
          setParkingSlots(parsedStatuses);
        }
      });
  ...
  const ParkingSlot: React.FC<ParkingSlotProps> = ({ status, index }) => (
    <View
      style={[
        styles.slot,
        {
          backgroundColor: status === "Empty" ? colors.green : colors.rose,
        },
      ]}
    >
      <Text style={styles.slotText}>{index + 1}</Text>
    ...
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.statusContainer}>
          ...
          <Text style={styles.availableText}>
            {countAvailableSpots(parkingSlots)} spots available
          </Text>
          {lastError && (
            <>
              <Text style={styles.errorText}>{lastError}</Text>
        ...
        {/* Parking Slots Grid */}
        <View style={styles.slotsGrid}>
          {parkingSlots.map((status, index) => (
            <ParkingSlot key={index} status={status} index={index} />
          ))}
        </View>
